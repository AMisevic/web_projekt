...
def is_valid_mock_coupon(code):
    """Mock validacija: valjan je ako je zbroj znamenaka 20."""
...
def top_up_balance(request):
    if request.method == "POST":
        ...
        if code:
            '''
            if not is_valid_mock_coupon(code):
                messages.error(request, 'Kod nije valjan!')
                return render(request, 'pawgoapp/nadoplate.html')
            '''
            try:
